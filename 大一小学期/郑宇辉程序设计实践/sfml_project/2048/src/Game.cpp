...
Game::Game(int w, int h) {
	tileSize =
		(w - FIELD_MARGIN * 2 - TILE_MARGIN * (FIELD_WIDTH - 1)) / FIELD_WIDTH;
	animState = false;

	if (!font.openFromFile("ClearSans.ttf"))
...
void Game::OnEvent(sf::Event e) {
	if (const auto* keyPressed = e.getIf<sf::Event::KeyPressed>()) {
		...
		sf::Keyboard::Key kc = keyPressed->code;

		if (kc == sf::Keyboard::Key::Left || kc == sf::Keyboard::Key::A)
			move(-1, 0);
		else if (kc == sf::Keyboard::Key::Right || kc == sf::Keyboard::Key::D)
			move(1, 0);
		else if (kc == sf::Keyboard::Key::Up || kc == sf::Keyboard::Key::W)
			move(0, -1);
		else if (kc == sf::Keyboard::Key::Down || kc == sf::Keyboard::Key::S)
			move(0, 1);
	}
}
...
void Game::Render(sf::RenderTarget& tgt) {
	...
	for (int x = 0; x < FIELD_WIDTH; x++) {
		for (int y = 0; y < FIELD_HEIGHT; y++) {
			tile.setPosition(
				{static_cast<float>(FIELD_MARGIN +
									x * (tileSize + TILE_MARGIN)),
				 static_cast<float>(FIELD_MARGIN +
									y * (tileSize + TILE_MARGIN))});
			...
			if (map[x][y] != 0) {
				...
				text.setPosition(
					{static_cast<float>(
						 FIELD_MARGIN + x * (tileSize + TILE_MARGIN) +
						 (tileSize - fr.size.x) / 2 - fr.position.x),
					 static_cast<float>(
						 FIELD_MARGIN + y * (tileSize + TILE_MARGIN) +
						 (tileSize - fr.size.y) / 2 - fr.position.y)});
	...
	for (int i = 0; i < moves.size(); i++) {
		...
		sf::Vector2i t =
			fMargin + moves[i].first.second * (tileSize + TILE_MARGIN);

		float movePercent =
			(animClock.getElapsedTime().asSeconds() / ANIMATION_DURATION);
		sf::Vector2f curPos =
			sf::Vector2f(f) + sf::Vector2f(t - f) * movePercent;
		...
		text.setPosition(
			{curPos.x + (tileSize - fr.size.x) / 2 - fr.position.x,
			 curPos.y + (tileSize - fr.size.y) / 2 - fr.position.y});
...
void Game::Reset() {
	for (int x = 0; x < FIELD_WIDTH; x++)
		for (int y = 0; y < FIELD_HEIGHT; y++) map[x][y] = 0;
	Spawn();
}
...
void Game::move(char dirX, char dirY) {
	// copy map
	for (int x = 0; x < FIELD_WIDTH; x++)
		for (int y = 0; y < FIELD_HEIGHT; y++) tempMap[x][y] = map[x][y];
...
void Game::applyMove(sf::Vector2i f, sf::Vector2i t, int dx, int dy) {
	...
	if (destVal == srcVal) {
		...
		tempMap[t.x - (dx * (destVal != 0))][t.y - (dy * (destVal != 0))] =
			srcVal;
	...
	if (destVal == srcVal)
		...
		to = sf::Vector2i(t.x - (dx * (destVal != 0)),
						  t.y - (dy * (destVal != 0)));

	if (from != to) {
		map[f.x][f.y] = 0;

		moves.push_back({{from, to}, srcVal});
